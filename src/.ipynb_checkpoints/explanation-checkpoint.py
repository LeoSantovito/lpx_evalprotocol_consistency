...
def explanations_to_json(paths_dict, data):
    ...
    for triple_test_index in paths_dict.keys():
        ...
        head = id_to_entity.get(int(test_triple[0]), str(test_triple[0]))
        rel = id_to_relation.get(int(test_triple[2]), str(test_triple[2]))

        for pred_index in paths_dict[triple_test_index].keys():
            predicted_tail_id = data.test_predicted_tails[triple_test_index][pred_index]
            predicted_tail = id_to_entity.get(int(predicted_tail_id), str(predicted_tail_id))
            explanations = paths_dict[triple_test_index][pred_index]

            if explanations != {None}:
                pred_entry = {
                    "pred": [head, rel, predicted_tail],
                    "explanation": []
                }

                for expl_type, expl_list in explanations.items():
                    for expl in expl_list:
                        ...
                        if len(path) == 3:  # Direct path [h, r, t]
                            ...
                            e = id_to_entity.get(int(path[2]), str(path[2]))
                            pred_entry["explanation"].append([h, r1, e])

                            # Second triple: e -r2-> t
                            r2 = id_to_relation.get(int(path[3]), str(path[3]))
...
def main(manager):
    ...
    log.info(f"Explanations saved to {output_file}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_level', dest='log_level', type=str,
                        help='set the logg--ing level, choose between info or debug',
                        default="info")
